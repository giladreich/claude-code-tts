...
Eight URLs exist in the entire source tree: five things to download, one file listing, and two documentation links that open in your browser only if you click them. You can confirm this yourself:
...
| Setting up Qwen3 or Chatterbox on Windows with an NVIDIA GPU | The build of PyTorch for that GPU from PyTorch's own package index, `download.pytorch.org/whl/...`, about 3 GB; the driver's CUDA version, read from `nvidia-smi` on this machine, picks the index and is sent nowhere | File GETs through uv | Yes: without an NVIDIA driver the package comes from PyPI as before, and runs on the CPU |
| A voice model is being fetched and the runtime wrote no file listing into its cache (the runtime on Windows) | The model's file listing from the hub, `huggingface.co/api/models/<model>/tree/main`, a few kilobytes, so the wait can say "1.4 of 4.2 GB" rather than "1.4 GB" | A GET to the host the weights are already coming from. Nothing about this machine is sent | Yes: it is only asked during a fetch you started; without it the bytes so far are shown without a percentage |
...
| `~/.claude/claude-code-tts-notify.json` | Notification sound choices (only if you enable notification sounds) |
...
- **Read the code.** `src/tts/net.ts` is the only HTTP client, and it is imported by exactly six files: `src/setup/kokoroSetup.ts` and `src/setup/piperSetup.ts` (both behind an explicit user action with a consent dialog that states the size), `src/platform/uvBootstrap.ts`, which downloads a pinned release of `uv` and refuses to unpack it unless it matches the SHA-256 that project publishes, `src/extension.ts`, which only hands it VSCode's `http.proxy` setting at activation and downloads nothing, `src/platform/modelProgress.ts`, which asks the hub for a model's file listing while that model is being fetched, so the wait can show a percentage, and `src/language/translate.ts`, which fetches a translation model from the address the Argos index names for it (the translation runtime used to fetch it itself, outside the editor's proxy and without progress).
