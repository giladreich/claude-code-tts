...
**Kokoro** is the cheapest to install: one 360 MB download into the extension's own storage, no Python, 28 English voices, and it speaks faster than realtime. Then **Select Voice** browses them with a sample on every highlight. The engine picker marks Kokoro "(recommended)" on a small machine (under 8 GB of memory, or at most two cores), and on every Windows and Linux machine, when it speaks the languages you listen in: there the cloning engines run on PyTorch and keep up only with a capable NVIDIA GPU (Qwen3 measured 0.75x realtime on a laptop and 8x slower than speech on the CPU; Chatterbox 2.4x slower even on the GPU). On Apple Silicon it recommends Qwen3, whose MLX runtime streams faster than realtime and also speaks as a voice you make.
...
| Qwen3 or Chatterbox is far behind Claude on a Windows or Linux machine | Two causes. On Windows the PyTorch build PyPI ships has no GPU support, so it runs on the CPU: **Check Setup** says so on the engine's row, and running the engine's setup again installs the build for the GPU. Beyond that, a GPU with little compute or an older driver: Qwen3 on PyTorch measured 0.75x realtime on a laptop with an NVIDIA GPU (its first word within two seconds, the sentences of a message generated together and played without gaps), Chatterbox 2.4x slower than speech even there. The "Claude Code TTS" output channel prints the measured speed on each sentence it buffers ("synthesis 0.8x realtime"). Kokoro and Piper keep up on any machine; the cloning engines are for a voice of your own. |
...
| Windows: "An Application Control policy has blocked this file" (WinError 4551) | Windows refused a program file not signed by a known publisher: Smart App Control on a personal machine, an App Control for Business policy on a managed one. Every neural engine here is built from open-source components that are not signed, so they run only while it is off; the built-in Windows voice is signed by Microsoft and keeps working, and the guided setup and the engine list say so before downloading anything. The setting is yours or your administrator's, not something this extension asks you to change: Smart App Control lives under Windows Security, App & browser control, and on current Windows builds it can be switched either way (its evaluation period blocks nothing). An administrator allowing the tools under a policy needs a rule for a user-writable folder (`%APPDATA%\uv\tools`), which App Control for Business only honours with its runtime file-path protection disabled, or the tools installed under a folder only administrators can write to. |
