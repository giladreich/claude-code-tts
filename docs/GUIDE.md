...
| Qwen3 or Chatterbox is far behind Claude on a Windows machine with an NVIDIA GPU | It is running on the CPU: the PyTorch build PyPI ships for Windows has no GPU support. **Check Setup** says so on the engine's row, and running the engine's setup again installs the build for the GPU. |
